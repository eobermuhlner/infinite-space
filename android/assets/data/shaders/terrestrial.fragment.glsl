...
float calculateHeight(vec2 P) {
	...
	height += fractalNoise(P, base, range);
		...
		float mountainFactor = smoothstep(0.0, 0.6, ((pnoise2(P+vec2(u_random6+u_random3, u_random1+u_random2), mountainFrequency) + 1.0) * 0.5));
...
vec3 planetColor(vec2 P, float height, float distEquator) {
	...
	float v2 = pnoise2(P+u_random1 + u_random7, 8.0) * 0.5 + 0.5;
